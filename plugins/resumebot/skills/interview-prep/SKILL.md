...
**Every named attendee gets anticipated questions — including the maybes.** A "may join if around" attendee still asks questions when they show, usually fast and without warm-up; a card plus a what-to-give-them plan is not coverage. Two or three anticipated questions per contingent attendee, same format as the confirmed ones.
...
Research-backed where possible (search for common questions for this role type and stage). For each anticipated question, lead with the **method** (the "how" the interviewer is really asking for), then assign **one story** with a checkable result. **Answer style is a per-user setting — check memory for it before coaching.** Some users persuade through stories; others lead with live judgment and use receipts only as one-clause anchors ("that's how I ran the risk register"), holding the full story until the interviewer asks. Coach in the user's mode, not the format's default: a judgment-led user forced to front-load biography reads as recitation, and a story-led user stripped of narrative reads as vague. If memory holds no style preference yet, learn it from the first mock answers and record it. Spread stories across engagements — hanging four answers on the same project sounds like a one-engagement career. Pull stories and STAR mechanics from the story bank if one exists (Notion or local), and use the user's own saved voice framings from memory before inventing generic formulas — their own framings beat textbook answers every time.
...
**Standing question habits live in memory, not here.** If the user has a signature question they like to open or close their segment with, memory will say so — include it as question zero (or last) for each interviewer in their preferred spot. One vetted pattern worth offering any user: the coverage check ("Is there anything you were looking for that we didn't cover?") as segment opener — it clears the interviewer's checklist while the candidate can still answer, costs fifteen seconds if the answer is no, and is NOT the banned "what reservations do you have about me" (it targets the conversation's coverage, not the candidate's standing). Whoever uses it, the session still ends on the candidate's landing line, not the interviewer's checklist.
...
**Feedback, per answer, kept short:** what landed, what to tighten, and — only if the answer missed — a one-line reframe in the user's own voice. Score against the report's own bar: did the answer lead with method, did the story carry a checkable result, did it survive the follow-up. Never coach a claim the consistency gate would reject; the gates bind the mock as much as the report. The same limit applies to knowledge, not just credentials: research facts from the prep do not transfer into the user's mouth. A line the user cannot personally defend under one follow-up is a fabrication with a coach's fingerprints on it, even when the fact is true — domain color from the report stays in the report; the user speaks only from what they hold. (Origin: mock coaching fed the user an industry-trend line from prep research; the user flagged they couldn't back it, 2026-08-20.)
