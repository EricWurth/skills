...
Research-backed where possible (search for common questions for this role type and stage). For each anticipated question, lead with the **method** (the "how" the interviewer is really asking for), then assign **one story** with a checkable result. The story is grounding, not the payload: in coaching and in the room, the user leads with judgment, the receipt is at most a one-clause anchor ("that's how I ran the risk register"), and the full story waits until the interviewer asks for it. Never coach the user to front-load biography — pulled stories persuade; pushed ones read as recitation. Spread stories across engagements — hanging four answers on the same project sounds like a one-engagement career. Pull stories and STAR mechanics from the story bank if one exists (Notion or local), and use the user's own saved voice framings from memory before inventing generic formulas — their own framings beat textbook answers every time.
...
**The user's standing segment opener.** The user likes to open their question segment with: "Is there anything you were looking for that we didn't cover?" Include it as question zero for each interviewer — it clears the interviewer's checklist while the user can still answer, costs fifteen seconds if the answer is no, and is NOT the banned "what reservations do you have about me" (it targets the conversation's coverage, not the candidate's standing). Pair it with the rule that the session still ends on the user's landing line, not the interviewer's checklist.
