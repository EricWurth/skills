...
6. **Gate:** present the sandboxed diff, regression result, and discrimination result. Promote only on explicit human sign-off, every time. Rejected/shelved candidates are logged in `technique-library.md`, not silently dropped.
7. **Report:** which skills were swept, what was found, what was tested, what was promoted or rejected and why, and the next suggested sweep date.

Invariants that survive any shortcut: refresh before identify; fitness traced to THIS skill's own documented history, not general technique reputation; exactly one candidate sandboxed per pass, per target skill; both regression and discrimination required before any gate decision; promotion always requires explicit human sign-off; a sweep that promotes nothing is a valid, expected outcome, not a shortfall.
