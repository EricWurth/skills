...
handled by `main.js`'s `handleMessage` plus `scanner.js` for the raw scan).
...
| `setSiteRegistry` | `data` (full registry) | `{ success }` |
...
| `scanResult` | `fields` (label/type only), `ats?` — sent BY content scripts | `{ success }`; updates the tab badge (summed across frames) |
...
```
{
  ...
  credentialStatus: { hasCredential, locked },   // INSIDE status, not a sibling
...
|---|---|---|---|
| `scanFromContent` | — | scanner.js | `{ success, fields }` raw field report |
...
| `fillCredentials` | `username`, `password` | main.js | `{ success, filled, passwordFields }` — the ONLY path that fills a password field |
...
1. Add it to the handler (`service-worker.js` switch, or `main.js` `handleMessage`) first.
