...
description: >
  This skill should be used when Eric asks for status on delegated work — "status",
...
Produce a status report that is a decision queue, not an inventory. If an item does not require a decision from Eric or inform one he would make, it does not appear.
...
**Approaching your threshold.** Early-warning items: a risk trending toward becoming an issue, an actual trending toward the order-of-magnitude flag, a fork decided close to the escalation line (stated so Eric can recalibrate judgment before it matters, not to reopen it).
...
- No task lists, no activity narration, no "risks have been logged". These are performative and measure nothing Eric steers by.
- Answer at the altitude of the question: if Eric asked a one-line question, the report may be one section or one sentence. The full format is for checkpoints, not every reply.
...
- Before reporting any discrepancy (between plan and reality, between Eric's expectation and outcome), restate the widest reasonable reading of the expectation first and check the discrepancy survives.
