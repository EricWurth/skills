...
tooling (web search/fetch) plus the bundled `references/report-template.html`.
It requires no external services, paid APIs, or other skills.
...
and that's by design; the whole point is doing more work than a quick
lookup would.
